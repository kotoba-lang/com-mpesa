(ns mpesa.main
  "Kotodama WASM entrypoint for the Mpesa clean-room actor (L4) — Clojure port.
  ...
  facts (`mpesa.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"mpesa.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mpesa-compat" :tier tier :entities entities} 200])
